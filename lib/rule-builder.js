...
var RuleBuilder = function( el ) {
  ...
  this.predicate_class = 'rule-builder-predicate';
  this.conjunction_class = 'rule-builder-conjunction';
  this.rule_options_class = 'rule-builder-rule-option';
  this.rule_remove_class = 'rule-builder-remove';
  this.rule_remove_button_class = 'rule-remove_button';
...
RuleBuilder.prototype = {
  ...
  add_rule_box: function( e ) {
    ...
    var structure = '<div class="' + this.rule_class + '">' + 
      '<div class="' + this.predicate_class + '">' +
        this.get_predicates() +
      '</div>' +
      '<div class="' + this.rule_options_class + '"> </div>' +
      '<div class="' + this.rule_remove_class + '">' +
        Util.dom.tag( 'a', '-', { href: '#', class: this.rule_remove_button_class } );
      '</div>' +
    '</div>';
  ...
  remove_rule_box: function( e ) {
    // to be implemented 
    var t    = $( e.target );
    var rule = t.parent( '.' + this.rule_remove_class ).parent( '.' + this.rule_class );
    var rule_parent = ( rule.parent( '.' + this.body_class ).length > 0 ) ? rule.parent( '.' + this.body_class ) : rule.parent( '.' + this.rule_class );

    //  Remove rule
    rule.remove();

    // Remove conjucntion if its the last 
    // item in the set
    rule_parent.children( '.' + this.conjunction_class + ':last-child' ).remove();
  ...
  handle_click: function( e ) {
    ...
    else if( target.hasClass( this.rule_remove_button_class ) ) {
      this.remove_rule_box( e );
    }
